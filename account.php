    ...
    if(isset($_POST['submit'])){
        ...
        if(!empty($username) and !empty($email)){
            if($user->username != $username and $getFromU->checkUsername($username) === true){
                $error['username'] = 'Имя пользователя некорректно!';
            } else if(preg_match("/[^a-zA-Z0-9\!]", $username)) {
                $error['username'] = 'Имя содержит недопустимые символы. Можно использовать только буквы и цифры.';
            ...
            $error['fields'] = 'Все поля обязательны к заполнению!';
        }
    }
?>
<!DOCTYPE HTML>
